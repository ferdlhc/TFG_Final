#---------------------------------------------------------#
#-------- SISTEMA SEGUIDOR SUDÁFRICA SIN COLUMNAS --------#
#---------------------------------------------------------#
...
#os.mkdir('C:/Users/ferdl/Documents/bifacial_radiance/TFG_Final/Seguimiento/Columnas/Sudafrica_NoColumnas')
os.chdir('C:/Users/ferdl/Documents/bifacial_radiance/TFG_Final/Seguimiento/Columnas/Sudafrica_NoColumnas')
path = os.getcwd()
print("Cambio de directorio de trabajo a:", path)

simulationName="Sudafrica_NoColumnas"
...
starttime_loop = pd.to_datetime('2021-03-20 7:0:0 +2')
endtime_loop =pd.to_datetime('2021-03-20 19:0:0 +2')
...
for timess in range (starttimeindex, endtimeindex+1):
    ...
    if j<endtimeindex+1:
        # Cálculo ángulo óptimo para seguidor
        tilt.append(ObjRad.getSingleTimestampTrackerAngle(metdata, timeindex=timess, gcr=gcr,
                                                          azimuth=tracker_azimuth, axis_tilt=tracker_axis_tilt,
                                                          limit_angle=tracker_limit_angle, backtrack=False))
    elif j==endtimeindex+1:
        tilt.append(-85.42)
        
    # Creación de Escena
    sceneDict.append({'tilt':tilt[j-1],'pitch':pitch,'hub_height':hub_height,'azimuth':azimuth_ang, 
                  'nMods': nMods, 'nRows': nRows, 'appendRadfile':True})
    sceneObj.append(ObjRad.makeScene(module=module_type, sceneDict=sceneDict[j-1], radname=module_type+f'_{j}'))
    ...
    results_df.append(bifacial_radiance.load.read1Result(f'results/irr_Sudafrica_NoColumnas_{j}.csv'))
...
with pd.ExcelWriter('C:/Users/ferdl/Documents/bifacial_radiance/TFG_Final/Seguimiento/Columnas/Resultados_Irradiancia/Sudafrica_NoColumnas.xlsx') as writer:
